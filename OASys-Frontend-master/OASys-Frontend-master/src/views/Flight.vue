            ...
            <el-table-column
                ...
                <template slot-scope="scope">
                    ...
                    <el-button type="danger" size="mini"
                               @click="deleteFlight(scope.row)">删除
                    </el-button>
                </template>
            </el-table-column>
        </el-table>
...
<script>
    import {addFlight, getFlight, updateFlight, deleteFlight} from "../utils/api";
                ...
                getFlight(this.searchParams).then(response => {
                    if (response && response.status === "success") {
                        ...
                        for(var i=0;i<this.flightList.length;i++){
                            var l = this.flightList[i]
                            if (l.depDate.length < 19){
                                l.depDate = l.depDate + " 00:00:00"
                            }
                            if (l.depTime.length < 19){
                                l.depTime = l.depTime + ":00"
                            }
                            if (l.arrTime.length < 19){
                                l.arrTime = l.arrTime + ":00"
                            }
                        }
                    }
                })
